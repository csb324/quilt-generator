...
module.exports = {
  sort_pixels: function(img_data, cb){
    console.log('sorting pixels...');

    fs.writeFile(__dirname + '/../temp.png', img_data, 'base64', function(err) {
      fs.createReadStream(__dirname + '/../temp.png')
        .pipe(new png({
            checkCRC: false
        }))
        .on('parsed', function() {
          ...
          this.pack().pipe(fs.createWriteStream(__dirname + '/../temp.png').on('finish', function(){
            var img_data = fs.readFileSync(__dirname + '/../temp.png', { encoding: 'base64' });
            cb(null, img_data); 
            })
          );
        });
